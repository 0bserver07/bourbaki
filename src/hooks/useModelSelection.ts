...
export function useModelSelection(
  ...
  const handleModelSelect = useCallback((modelId: string | null) => {
    ...
    // Build full model ID with provider prefix where needed
    const getFullModelId = (provider: string, model: string): string => {
      switch (provider) {
        ...
        default:
          // OpenAI, Anthropic, Google, xAI use model name directly
          return model;
      }
    };

    const fullModelId = getFullModelId(pendingProvider, modelId);

    // For local Ollama, skip API key flow entirely
    ...
    if (checkApiKeyExistsForProvider(pendingProvider)) {
      ...
      // Need to get API key - store the selected model temporarily (with prefix)
